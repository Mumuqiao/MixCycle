...
def generate_mixpc(pc, mix_pc, pc_box, mix_box, mix_rate,offset=0.0,scale=1.0,seed = None):
    new_PC = crop_pc_axis_aligned(pc, pc_box, offset=offset, scale=scale)

    if mix_rate == 1.0:
        return new_PC,pc_box, mix_rate
    new_box = copy.deepcopy(pc_box)
    ...
    # crop around box
    croped_pc, close_pc = crop_pc_axis_aligned(new_PC, new_box, offset=0.0, scale=1.0,return_mask=True)
    croped_mix_pc,_= cropAndCenterPC(mix_pc,mix_box,offset=0.0,scale=1.0)
    if croped_mix_pc.nbr_points() == 0:
        return new_PC,pc_box, 1.0
    pc_num = int(croped_pc.nbr_points() * mix_rate)
    mix_num = int(croped_pc.nbr_points() - pc_num)
    
    rng = np.random if seed is None else np.random.default_rng(seed)

    close_pc = np.logical_not(close_pc)
    new_pc = new_PC.points[:,close_pc]
    new_pts_idx = None

    if pc_num > 0:
        new_pts_idx = rng.choice(croped_pc.nbr_points(), size=pc_num, replace=False)
    if new_pts_idx is not None:
        croped_pc = croped_pc.points[:, new_pts_idx]
        if len(new_pc.shape) != len(croped_pc.shape):
            croped_pc = np.expand_dims(croped_pc,axis=1)
        new_pc = np.concatenate([new_pc, croped_pc],axis=1)
    new_pts_idx = None
    if mix_num > 0:
        new_pts_idx = rng.choice(croped_mix_pc.nbr_points(), size=mix_num, replace=mix_num > croped_mix_pc.nbr_points())
    if new_pts_idx is not None:
        croped_mix_pc = croped_mix_pc.points[:, new_pts_idx]
        if len(new_pc.shape) != len(croped_mix_pc.shape):
            croped_mix_pc = np.expand_dims(croped_mix_pc,axis=1)
        new_pc = np.concatenate([new_pc, croped_mix_pc],axis=1)
    mixed_pc = PointCloud(new_pc)
    mixed_box = copy.deepcopy(pc_box)
    mixed_box.wlh = pc_box.wlh * mix_rate + mix_box.wlh * (1.0 - mix_rate)
    return mixed_pc, mixed_box, mix_rate

def generate_mixpc_template(pc, mix_pc, pc_box, mix_box, mix_rate,offset=0.0,scale=1.0,seed = None):
    if mix_rate == 1.0:
        return pc,pc_box, mix_rate
    ...
    if croped_mix_pc.nbr_points() == 0:
        return new_PC,pc_box, 1.0
    pc_num = int(new_PC.nbr_points() * mix_rate)
    mix_num = int(new_PC.nbr_points() - pc_num)
    ...
    if pc_num > 0:
        new_pts_idx = rng.choice(new_PC.nbr_points(), size=pc_num, replace=False)
    if new_pts_idx is not None:
        new_PC = new_PC.points
        new_PC = new_PC[:, new_pts_idx]
        if len(new_PC.shape) != 2:
            new_PC = np.expand_dims(new_PC,axis=1)
    else:
        new_PC = None
    new_pts_idx = None
    if mix_num > 0:
    ...
    if new_pts_idx is not None:
        croped_mix_pc = croped_mix_pc.points[:, new_pts_idx]
        if len(croped_mix_pc.shape) != 2:
            croped_mix_pc = np.expand_dims(croped_mix_pc,axis=1)
        if new_PC is not None:
            new_PC = np.concatenate([new_PC, croped_mix_pc],axis=1)
        else:
            new_PC = croped_mix_pc
    mixed_pc = PointCloud(new_PC)
    mixed_box = copy.deepcopy(pc_box)
    mixed_box.wlh = pc_box.wlh * mix_rate + mix_box.wlh * (1.0 - mix_rate)
    return mixed_pc, mixed_box, mix_rate
...
def getClose(pc,box,offset=0,scale=1.0):
