...
class Filerts:
    def __init__(self,filername,size,n_comp=5, dim=3) -> None:
        if 'kalman'.upper() in filername.upper():
            self.Filter_list = [KalmanFiltering() for _ in range(size)]
        elif ("Particle".upper() in filername.upper()):
            self.Filter_list = [ParticleFiltering() for _ in range(size)]
        elif ("GMM".upper() in filername.upper()):
            self.Filter_list = [GaussianMixtureModel(n_comp,dim) for _ in range(size)]
        else:
            self.Filter_list = [ExhaustiveSearch() for _ in range(size)]
    
    def sample(self, n=10):
        result  = [None]* len(self.Filter_list)
        for idx in range(len(self.Filter_list)):
            result[idx] = self.Filter_list[idx].sample(n)
        return result


    def addData(self, data,score):
        for i in range(len(score)):
            sc = np.array([score[i]]).T
            self.Filter_list[i].addData(data[i],sc)

    def reset(self):
        for KF in self.Filter_list:
            KF.reset()