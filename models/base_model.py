""" 
baseModel.py
modified by Qiao Wu at 2022/9/1 16:29
"""
...
class SGDscheduler(_LRScheduler):
    def __init__(self, optimizer: Optimizer, last_epoch = -1, total_epoch = 60,verbose=False) -> None:
        self.total_eopch = total_epoch
        self.lr_table = torch.logspace(-2,-5,self.total_eopch+1)
        super(SGDscheduler, self).__init__(optimizer, last_epoch, verbose)
    ...
    def get_lr(self) -> float:
        ...
        return [self.lr_table[self.last_epoch]
...
class BaseModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        if self.config.optimizer.lower() == 'sgd':
            ...
            # torch.optim.lr_scheduler.CyclicLR(optimizer, 5e-4, 1e-1, step_size_up=10, step_size_down=10, mode='triangular', gamma=1.0, scale_fn=None, scale_mode='cycle', cycle_momentum=True, base_momentum=0.8, max_momentum=0.9, last_epoch=-1)
            # scheduler = SGDscheduler(optimizer,total_epoch=self.config.epoch)
        else:
    ...
    def generate_template(self, sequence, current_frame_id, results_bbs):
        ...
        if "firstandprevious".upper() in self.config.shape_aggregation.upper():
            ...
            template_pc, canonical_box = points_utils.cropAndCenterPC(first_pc, results_bbs[0],
                                                                      scale=self.config.model_bb_scale,
                                                                      offset=self.config.model_bb_offset)
        elif "previous".upper() in self.config.shape_aggregation.upper():
